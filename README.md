...
```bash
claude plugin install github:Preetam3620/chatbot-creator-plugin
```

Or test locally without installing:

```bash
claude --plugin-dir ./chatbot-creator-plugin
```
...
Open Claude Code in an empty project folder and invoke the skill:

```
/chatbot-creator:chatbot-creator
```

If `backend/` or `frontend/` already exist, Claude will list the files that would be overwritten and ask for confirmation before proceeding.
...
skills/
  chatbot-creator/
    SKILL.md                ← Skill instructions (what Claude reads at invocation)
    references/
